...
最近因为突发奇想对并发编程不知道哪就来了兴趣，然后想来Go是现在非常炙手可热的一门并行编程语言，而Erlang虽然有其迷人之处，但是这么纯正的函数式语言要学起来实在是曲线也有点陡。

OK不扯了，打算边学边写一点文章介绍下Go
...
slice用起来和Python里的数组有点像，可以把它理解为数组的一个view，而真正存放元素的是数组，但是**slice是引用**（此处有回音）。
